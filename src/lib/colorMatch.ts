import type { MatchResult, Mood, Profile, Song } from '../types'
...
export async function extractImageColor(imageUrl: string): Promise<string | undefined> {
  try {
...
export function matchProfile(profile: Profile): MatchResult {
  ...
  const reasons = [`Your three-photo palette leans ${palette}.`, `Your profile adds ${profileSignals.length ? profileSignals.slice(0, 2).map((signal) => signal.split(' → ')[0]).join(' + ') : 'a quiet, neutral energy'}.`, `${winner.title} matches the ${winner.mood} feeling best.`]
  return { colors, palette, mood: winner.mood, song: winner, profileSignals, reasons }
}
